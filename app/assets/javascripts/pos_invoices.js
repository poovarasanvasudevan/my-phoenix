...
$(document).on('click', '#save_button', function (event) {
    ...
    //return event.preventDefault();
    return getSoldQuantity(postedData1);
});

function getSoldQuantity(params) {

    var myParams = JSON.stringify(params);
    var notAvailable = [];
    var finalResult = false;
    $.post("/total-sales-summary-validate.json", {'data': myParams}, function (data) {
        alert(data);
        ...
        if (notAvailable.length > 0) {
            ...
            var textDisplay = "Items With SKU ";
            $.each(notAvailable, function (i, field) {
                textDisplay = textDisplay + " " + field;
            });
            textDisplay = textDisplay + " " + " Is Going to Negative Sales";
            textDisplay = textDisplay + " " + "\n Are You Sure You Want to Make Sale ? ";

            alert(textDisplay);

            bootbox.confirm(textDisplay, function (result) {
                if (result)
                    return true;
                else
                    return false;
